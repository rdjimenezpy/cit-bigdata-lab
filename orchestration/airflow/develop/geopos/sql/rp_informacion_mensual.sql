...
BEGIN
    ...
    DROP TABLE IF EXISTS analytic.rp_informacion_mensual;

    -- Crear la tabla directamente fuera de los CTEs
    CREATE TABLE analytic.rp_informacion_mensual AS
    ...
    temp_infomensual AS (
        ...
        GROUP BY "OpLocal", "Periodo", "Anho", "Mes"
    )
    SELECT * FROM temp_infomensual;
END $$;