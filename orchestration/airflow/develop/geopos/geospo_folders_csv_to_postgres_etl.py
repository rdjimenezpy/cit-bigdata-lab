"""Descripción del DAG: ETL de multiples directorios con CSV a GEOPOS
...
# [START PROCESS]
with DAG(
        dag_id='geospo_folders_csv_to_postgres_etl_id',
        dag_display_name='geospo_folders_csv_to_postgres_etl',
        description='ETL de CSV a GEOPOS',
        default_args=default_args,
        template_searchpath=template_searchpath,
        schedule_interval=None,
        catchup=False,
        tags=['cotizacion_mensual', 'geopos', 'folders_csv']
) as dag:
    start = EmptyOperator(task_id="start")

    with TaskGroup("csv_to_ods", tooltip="Tareas de extracción de datos") as csv_to_ods_task:
        ods.task_folders_csv_to_ods()
